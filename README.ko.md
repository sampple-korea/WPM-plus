...
- Samsung Quick Share `WiFi_*.json.gz`, JSON, CSV, Wi‑Fi QR 가져오기
- Shizuku/Sui 기반 shell/root 권한 추출 엔진
...
| Shizuku ADB shell | shell 권한으로 가능한 Wi‑Fi 진단/명령 | production build에서는 보통 SSID만 가능 |
| Shizuku root / Sui | root로 읽을 수 있는 Wi‑Fi config store 파일 | 알려진 시스템 파일에서 PSK를 best-effort로 추출 |
